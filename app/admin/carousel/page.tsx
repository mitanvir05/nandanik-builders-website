...
import { FiArrowUp, FiArrowDown, FiStar, FiTrash2, FiPlus, FiX, FiUploadCloud } from "react-icons/fi";
import { getSlides, toggleFeatured, updateSlideOrder, deleteSlide, addSlide } from "@/actions/carousel.actions";
...
export default function CarouselManager() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        setFormData((prev) => ({ ...prev, imageUrl: data.secure_url, imageId: data.public_id }));
      } else {
  ...
  const moveSlide = async (index: number, direction: "up" | "down") => {
    ...
    await updateSlideOrder(updatedOrder.map(s => ({ _id: s._id, order: s.order })));
  };

  if (isLoading) return <div className="p-8 text-slate-500 font-medium text-center">Loading carousel data...</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Manage Carousel</h1>
          <p className="text-slate-500 text-sm mt-1">Add, reorder, and feature slides for the homepage.</p>
        </div>
      ...
      <div className="block md:hidden space-y-4">
        {slides.map((slide, index) => (
          <div key={slide._id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
            <div className="flex gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-slate-900 text-sm truncate">{slide.title}</h3>
                <p className="text-xs text-slate-500 line-clamp-1">{slide.description}</p>
              </div>
            </div>
            
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
               <div className="flex items-center space-x-2">
                  <button onClick={() => moveSlide(index, "up")} disabled={index === 0} className="p-2 text-gray-400 hover:text-blue-600 cursor-pointer disabled:opacity-20"><FiArrowUp size={18} /></button>
                  <span className="text-sm font-bold w-4 text-center">{index + 1}</span>
                  <button onClick={() => moveSlide(index, "down")} disabled={index === slides.length - 1} className="p-2 text-gray-400 hover:text-blue-600 cursor-pointer disabled:opacity-20"><FiArrowDown size={18} /></button>
               </div>
               ...
               <div className="flex items-center space-x-3">
                  ...
                  >
                    <FiStar className={slide.isFeatured ? "fill-current" : ""} size={18} />
                  </button>
                  <button 
                    onClick={async () => {
              ...
              <tr key={slide._id} className="hover:bg-gray-50/50 transition-colors group">
                <td className="p-4">
                  <div className="flex flex-col items-center space-y-1">
                    <button onClick={() => moveSlide(index, "up")} disabled={index === 0} className="p-1 text-gray-400 hover:text-blue-600 disabled:opacity-30 cursor-pointer"><FiArrowUp size={16} /></button>
                    <span className="text-sm font-bold text-slate-700">{index + 1}</span>
                    <button onClick={() => moveSlide(index, "down")} disabled={index === slides.length - 1} className="p-1 text-gray-400 hover:text-blue-600 disabled:opacity-30 cursor-pointer"><FiArrowDown size={16} /></button>
                  </div>
                </td>
                <td className="p-4">
                  <div className="relative w-24 h-16 rounded-md overflow-hidden bg-gray-100 border border-gray-200">
                    <Image src={slide.imageUrl} alt={slide.title} fill className="object-cover" sizes="96px" />
                  </div>
                </td>
                <td className="p-4">
                  <h3 className="font-bold text-slate-900 text-sm">{slide.title}</h3>
                  <p className="text-sm text-slate-500 line-clamp-2 mt-1 pr-4">{slide.description}</p>
                </td>
                <td className="p-4 text-center">
                  ...
                  >
                    <FiStar className={slide.isFeatured ? "fill-current" : ""} size={12} />
                    <span>{slide.isFeatured ? "Featured" : "Hidden"}</span>
                  </button>
                </td>
                <td className="p-4 text-center">
                  <button onClick={async () => { if (confirm("Delete?")) { setSlides(slides.filter(s => s._id !== slide._id)); await deleteSlide(slide._id); } }} className="p-2 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors cursor-pointer">
                    <FiTrash2 size={18} />
                  </button>
                </td>
              </tr>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-gray-100 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center p-5 border-b border-gray-100 sticky top-0 bg-white z-10">
              <h2 className="text-lg font-bold text-slate-900">Add New Slide</h2>
              <button onClick={() => setIsModalOpen(false)} className="text-gray-400 hover:text-gray-600 transition-colors cursor-pointer p-1">
                <FiX size={24} />
              </button>
            </div>

            <form onSubmit={handleAddSlide} className="p-5 space-y-5">
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-slate-500 mb-2">Slide Image</label>
                {formData.imageUrl ? (
                  <div className="relative w-full h-40 sm:h-48 rounded-xl overflow-hidden border border-gray-200 group">
                    <Image src={formData.imageUrl} alt="Preview" fill className="object-cover" />
                    <button type="button" onClick={() => setFormData({ ...formData, imageUrl: "", imageId: "" })} className="absolute inset-0 bg-slate-900/40 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity font-bold cursor-pointer">Remove Image</button>
                  </div>
                ) : (
                  <div className="relative w-full h-32 border-2 border-dashed border-blue-200 rounded-xl flex flex-col items-center justify-center text-blue-600 bg-blue-50/30 hover:bg-blue-50 transition-colors overflow-hidden cursor-pointer group">
                    ...
                    <FiUploadCloud size={28} className="mb-2 group-hover:scale-110 transition-transform" />
                    <span className="text-xs font-bold uppercase tracking-wide">{isUploadingImage ? "Uploading..." : "Upload Image"}</span>
                  </div>
                )}
              </div>
